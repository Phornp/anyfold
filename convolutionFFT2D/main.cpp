...
bool test0(void)
{
    ...
    *d_KernelSpectrum;
    ...
    const int    fftW = snapTransformSize(dataW + kernelW - 1);
        const int   fftD = snapTransformSize(dataD + kernelD -1);

    printf("...allocating memory\n");
    h_Data      = (float *)malloc(dataH   * dataW * dataD * sizeof(float));
    h_Kernel    = (float *)malloc(kernelH * kernelW * kernelD * sizeof(float));
    h_ResultCPU = (float *)malloc(dataH   * dataW * dataD * sizeof(float));
    h_ResultGPU = (float *)malloc(fftH    * fftW * fftD * sizeof(float));

    checkCudaErrors(cudaMalloc((void **)&d_Data, dataH * dataW * dataD * sizeof(float)));
    checkCudaErrors(cudaMalloc((void **)&d_Kernel, kernelH * kernelW * kernelD * sizeof(float)));

    checkCudaErrors(cudaMalloc((void **)&d_PaddedData,   fftH * fftW * fftD * sizeof(float)));
    checkCudaErrors(cudaMalloc((void **)&d_PaddedKernel, fftH * fftW * fftD * sizeof(float)));
    ...
    for (int i = 0; i < dataH * dataW * dataD; i++)
    {
        h_Data[i] = getRand();
    }

    for (int i = 0; i < kernelH * kernelW * kernelD; i++)
    {
        h_Kernel[i] = getRand();
    }
    ...
    checkCudaErrors(cufftPlan3d(&fftPlanFwd, fftH, fftW, fftD, CUFFT_R2C));
    checkCudaErrors(cufftPlan3d(&fftPlanInv, fftH, fftW, fftD, CUFFT_C2R));
    
    printf("...uploading to GPU and padding convolution kernel and input data\n");
    checkCudaErrors(cudaMemcpy(d_Kernel, h_Kernel, kernelH * kernelW * kernelD * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_Data, h_Data, dataH * dataW * dataD * sizeof(float), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemset(d_PaddedKernel, 0, fftH * fftW * fftD * sizeof(float)));
    checkCudaErrors(cudaMemset(d_PaddedData,   0, fftH * fftW * fftD * sizeof(float)));
